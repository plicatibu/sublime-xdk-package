...
class XDKPluginCore:
	...
	def make_request(self, addr, params={}, headers={}):
		...
		request = urllib2.Request(addr, params.encode('utf-8'), headers)
	...
	def authorize(self):
		...
		_print('authorize: response.code=' + str(response.code))
		cookies = response.headers
		if response.code != 200 or 'Set-Cookie' not in cookies: 
	...
	def prepare(self):
		try:
			...
			if isinstance(e, (urllib2.HTTPError, urllib2.URLError)):
				sublime.error_message(MSGS['XDK_CONNECTION_FAILED'])
	...
	def prepare_request_data(self):
		...
		_print('prepare_request_data: folder=' + folder)
		_print('prepare_request_data: filename=' + self.view.file_name())
